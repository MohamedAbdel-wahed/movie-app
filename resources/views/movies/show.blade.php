...
@section('content')
    ...
    <!--Movie Info -->
  <div class="container mx-auto border-b border-gray-700 py-12 px-6 mt-24">
      <div class="flex flex-col md:flex-row justify-between">
          <div class="w-full md:w-5/12 xl:w-1/3 flex ">
             <div class="flex sm:mr-56 md:mr-0 px-10 sm:px-4 xl:justify-end">
                <img src="https://image.tmdb.org/t/p/w500/{{$movie['poster_path']}}" class="rounded-sm w-full sm:w-10/12 md:w-full " style="height:26rem;">
             </div>
          </div>
          <div class="w-full md:w-7/12 xl:w-2/3 flex  sm:justify-start justify-center ">
                <div class="mt-6 md:mt-1 sm:px-4 px-10">
                   <div id="info">
                        <h1 class="capitalize text-white font-bold text-2xl md:text-3xl tracking-tight md:tracking-wide leading-tight">{{$movie['title']}}</h1>
                    ...
                    <div id="description" class="mt-6 xl:w-10/12 w-full px-2">
                          <p class="text-gray-300 text-xs">{{$movie['overview']}}</p>
                    </div>
                    ...
                    <div class="px-2 mt-8">
                        @if(count($movie['videos']['results'])>0)
                            <a href="{{'https://www.youtube.com/watch?v='.$movie['videos']['results'][0]['key']}}" class="inline-flex items-center text-gray-400 sm:text-2xl text-lg font-bold rounded-lg px-6 sm:px-10 py-3 bg-red-700 capitalize hover:opacity-75 hover:text-gray-700 focus:shadow:sm focus:outline-none">
   ...
   <!-- Movie Cast-->
                ...
                @foreach ($cast as $person)
                    @if($person['profile_path'])
                        <div class="flex flex-col items-center w-1/2 sm:w-4/12 md:w-3/12 lg:w-2/12 py-6">
                            <a href="/actor/{{$person['id']}}" class="flex justify-center">
    ...
    <div class="container mx-auto px-4 pb-16 pt-10 select-none border-b border-gray-700" >
        ...
        @else 
            <div class="mx-auto text-center mt-8">
                <h1 class="text-gray-400 4xl">No Images</h1>
            </div>
        @endif
    </div>
@endsection   